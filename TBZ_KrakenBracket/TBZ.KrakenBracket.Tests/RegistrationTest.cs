...
namespace TBZ.RegistrationTest
{
    [TestClass]
    public class RegistrationTest
    {
        ...
        public void Registration_validEmailAndPass_Pass()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < corrects.Length; i++)
                {
                    try
                    {
                        Account = new Registration(userNames[i], userPasswds[i], userFNames[i], userLNames[i]);
        ...
        public void Registration_validEmailGauntlet_Pass()
        {
            ...
            try
            {
                for (int i = 0; i < corrects.Length; i++)
                {
                    try
                    {
                        var Account = new Registration(emails[i], "Wid#$%766", "Bob", "Simon");
        ...
        public void Registration_validPasswordGauntlet_Pass()
        {
            ...
            try
            {
                for (int i = 0; i < passwds.Length; i++)
                {
                    try
                    {
                        var tempAccount = new Registration("personman1000@online.net", passwds[i], "Bob", "Simon");
                        if (corrects[i] != true) result = false;
                    }
                    catch (ArgumentException)
                    {
        ...
        public void getMethods_Pass()
        {
            ...
            try
            {
                var tempAccount = new Registration(usrEmail, usrPasswd, usrFName, usrLName);
